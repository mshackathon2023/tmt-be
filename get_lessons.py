...
GetLessons = func.Blueprint()

@GetLessons.route(route="getlessons/{topic}", methods=["GET"])
def getlessons(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        items = list(container.query_items(
            query="SELECT c.lessons FROM c WHERE c.id=@id",
            parameters=[
